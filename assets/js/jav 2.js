...
const hargaran = document.querySelectorAll('.hargaran')
let lunt = 0
hargaran.forEach(haha => {
    $(haha).html('US$'+(Math.floor((Math.random() * 295)) + 120))
})
reviews.forEach(rev => {
...
function changeCurrent(prev,current){
    ...
    if(prev =='USD' && current == 'EUR'){
        price.forEach(prc =>{
            ...
            $(prc).html(`EUR€${Math.round(harga*0.93).toLocaleString()}`)
        })
    }
    else if(prev =='USD' && current == 'IDR'){
        price.forEach(prc =>{
            const harga = ($(prc).html()).slice(3).split(',').join('')
            $(prc).html(`IDR${Math.round(harga*16254.0000).toLocaleString()}`)
        })
    }
    else if(prev =='EUR' && current == 'USD'){
        price.forEach(prc =>{
            const harga = ($(prc).html()).slice(4).split(',').join('')
            $(prc).html(`US$${Math.round(harga*1.08).toLocaleString()}`)
        })
    }
    else if(prev =='EUR' && current == 'IDR'){
        price.forEach(prc =>{
            const harga = ($(prc).html()).slice(4).split(',').join('')
            $(prc).html(`IDR${Math.round(harga*17617.0604).toLocaleString()}`)
        })
    }
    else if(prev =='IDR' && current == 'USD'){
        price.forEach(prc =>{
            const harga = (($(prc).html()).slice(3)).split(',').join('')
            $(prc).html(`US$${Math.round(harga*0.0000614232558).toLocaleString()}`)
        })
    }
    else if(prev =='IDR' && current == 'EUR'){
        price.forEach(prc =>{
            const harga = (($(prc).html()).slice(3)).split(',').join('')
            $(prc).html(`EUR€${Math.round(harga*0.0000567627223).toLocaleString()}`)
        })
    }
    let simbol = ''
    if(current == 'IDR'){
...
function changeCurrentOnly(prev,current,value){
    if(prev =='USD' && current == 'EUR'){
        return Math.floor(value*0.93)
    }
    else if(prev =='USD' && current == 'IDR'){
        return Math.floor(value*16322.50)
    }
    else if(prev =='EUR' && current == 'USD'){
        return Math.floor(value*1.08)
    }
    else if(prev =='EUR' && current == 'IDR'){
        return Math.floor(value*17554.80)
    }
    else if(prev =='IDR' && current == 'USD'){
        return Math.floor(value*0.000061)
    }
    else if(prev =='IDR' && current == 'EUR'){
        return Math.floor(value*0.00005696)
    }
}
...
$(document).on('input',function(e){
    ...
    if($(e.target).hasClass('Rooom')){
        e.preventDefault()
        const startingPrice = $(e.target).data('price')
        const curr = $(e.target).data('curr')
        if($(e.target).val() == 'sgl-bed'){
            let harga = (currentCurrency == curr ? startingPrice : changeCurrentOnly(curr,currentCurrency,startingPrice))
            let simbol = ''
            if(currentCurrency == 'IDR'){
                simbol = 'IDR'
            }
            ...
            $($($(e.target).parent()).next()).html(simbol + Math.floor(harga).toLocaleString())
            totalin('hotels')
        }
        else if($(e.target).val() == 'dbl-bed'){
            let harga = (currentCurrency == curr ? startingPrice : changeCurrentOnly(curr,currentCurrency,startingPrice)) * 1.5
            let simbol = ''
            if(currentCurrency == 'IDR'){
                simbol = 'IDR'
            }
            ...
            let harga = (currentCurrency == curr ? startingPrice : changeCurrentOnly(curr,currentCurrency,startingPrice)) * 8
            let simbol = ''
            if(currentCurrency == 'IDR'){
                simbol = 'IDR'
            }
            ...
            let harga = (currentCurrency == curr ? startingPrice : changeCurrentOnly(curr,currentCurrency,startingPrice)) * 12
            let simbol = ''
            if(currentCurrency == 'IDR'){
                simbol = 'IDR'
            }
            ...
            let harga = (currentCurrency == curr ? startingPrice : changeCurrentOnly(curr,currentCurrency,startingPrice)) * 0.6
            let simbol = ''
            if(currentCurrency == 'IDR'){
                simbol = 'IDR'
            }
            ...
            let harga = (currentCurrency == curr ? startingPrice : changeCurrentOnly(curr,currentCurrency,startingPrice)) * 0.8
            let simbol = ''
            if(currentCurrency == 'IDR'){
                simbol = 'IDR'
            }
...
$(document).click(function(e){
    ...
    if($(e.target).hasClass('productim')){
        ...
        let price = $(e.target).data('starting').split(' ')[4].split('>')[1].split('<')[0]
        if(theCurent == 'IDR'){
            price = price.slice(3).split(',').join('')
        }
        else if(theCurent == "EUR"){
            price = price.slice(4).split(',').join('')
        }
        else{
            price = price.slice(3).split(',').join('')
        }
    ...
    if($(e.target).hasClass('kmrmin')){
        ...
        for(let i = 0; i < $($(e.target).next()).html();i++){
            ...
            <tr>
                <td colspan="5"></td>
                <th>Room ${i+1}</th>
                <td><select name="type" id="" data-price='${startingPrice}' data-curr='${curr}' class="Rooom ${darkModeOn ? 'dark-mode' : ''}">
